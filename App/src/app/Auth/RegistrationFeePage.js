...
const RegistrationFeePage = () => {
  ...
  // State for image selection (same as RegisterPage2)
  ...
  useEffect(() => {
    ...
    // Detect browser and platform information (same as RegisterPage2)
    if (Platform.OS === 'web') {
  ...
  // Request permissions (same as RegisterPage2)
  useEffect(() => {
    (async () => {
      if (Platform.OS !== 'web') {
        try {
          ...
          if (cameraStatus !== 'granted' || libraryStatus !== 'granted') {
            setAlertMessage('Camera and gallery permissions are required for image uploads');
            setAlertType('error');
  ...
  const handleGallerySelection = async () => {
    ...
    try {
      ...
      setAlertMessage('Failed to select image from gallery. Please try again.');
      setAlertType('error');
      setAlertModalVisible(true);
    }
  ...
  const handleWebCameraCapture = (imageType) => {
    return new Promise((resolve) => {
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        setAlertMessage('Camera not supported in this browser. Please use gallery instead.');
        setAlertType('error');
        setAlertModalVisible(true);
        resolve(null);
        return;
      ...
      .then((stream) => {
        ...
        setAlertMessage('Camera not available. Please use gallery instead.');
        setAlertType('error');
        setAlertModalVisible(true);
        resolve(null);
      });
          ...
          >
            {proofOfPayment ? (
              ...
              <View style={styles.iconContainer}>
                <MaterialIcons name="add" size={40} color="#1E3A5F" />
...
const styles = StyleSheet.create({
  ...
  // Modal styles (same as RegisterPage2)
  modalBackground: {
    flex: 1,
