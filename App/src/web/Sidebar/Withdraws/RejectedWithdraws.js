...
const RejectedWithdraws = () => {
  const [rejectedWithdraws, setRejectedWithdrawsRef] = useState([]);
  ...
  useEffect(() => {
    const rejectedWithdrawsRef = ref(database, 'RejectedWithdraws'); // Path to RejectedWithdraws table
    const unsubscribe = onValue(rejectedWithdrawsRef, (snapshot) => {
      ...
      setRejectedWithdrawsRef(withdrawsArray);
      setLoading(false); // Set loading to false after data is fetched
    });

    return () => unsubscribe();
  }, []);
  const tableHead = ['Member ID',  'Withdraw Amount', 'Disbursement', 'Account Name', 'Account Number',  'Date Applied', 'Transaction ID', 'Date Rejected'];

  // Filter deposits based on search query
const filteredRejectedWithdraws = rejectedWithdraws.filter(item =>
...
// Pagination logic
const paginatedData = filteredRejectedWithdraws.slice(currentPage * pageSize, (currentPage + 1) * pageSize);
const totalPages = Math.ceil(filteredRejectedWithdraws.length / pageSize);
  ...
  return (
    ...
    <View style={styles.searchContainer}>
      ...
      ) : filteredRejectedWithdraws.length === 0 ? ( // Check if there are no withdrawals
        <View  style={styles.noDataMessage}>
          <Text>No Rejected Withdrawals Available</Text>
        </View>
      ) : (
        <ScrollView>
...
export default RejectedWithdraws;