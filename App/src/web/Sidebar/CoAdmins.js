...
const CoAdmins = () => {
  ...
  const handleAddAdmin = async () => {
    ...
    try {
      ...
      const highestAdminID = admins.length > 0
        ? Math.max(...admins.map(admin => parseInt(admin.id.replace('coadmin', '')) || 0))
        : 0;
      const newAdminID = 'coadmin' + (highestAdminID + 1);
      const dateAdded = new Date().toLocaleString();
  
      const adminRef = ref(db, 'Users/CoAdmin/' + newAdminID);
  ...
  const handleDeleteAdmin = () => {
    const db = getDatabase();
    const adminRef = ref(db, 'Users/CoAdmin/' + adminToDelete);
  ...
  return (
    <View style={{ padding: 40 }}>
      <Text style={{ fontSize: 30, fontWeight: 'bold', marginBottom: 20 }}>Co-Admins</Text>

      <View style={styles.topContainer}>
        <TouchableOpacity style={styles.addAdminButton} onPress={() => setModalVisible(true)}>
          <Text style={styles.addAdminButtonText}>Create New Co-Admin</Text>
        </TouchableOpacity>
      ...
      <Modal animationType="slide" transparent visible={modalVisible}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <Text style={styles.modalTitle}>New Co Admin</Text>
