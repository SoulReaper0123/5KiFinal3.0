...
const DataMigration = () => {
  ...
  const validateFields = () => {
    ...
    if (!formData.memberId.trim()) {
      ...
      isValid = false;
    } else if (!/^\d+$/.test(formData.memberId)) {
      setMemberIdError('Member ID must be a number');
      isValid = false;
    } else {
    ...
    if (!formData.currentBalance.trim()) {
      setBalanceError('Current balance is required');
      isValid = false;
    } else if (!/^\d+(\.\d{1,2})?$/.test(formData.currentBalance)) {
      setBalanceError('Current balance must be a valid number');
      isValid = false;
    } else if (parseFloat(formData.currentBalance) < 0) {
    ...
    if (!formData.currentInvestment.trim()) {
      setInvestmentError('Current investment is required');
      isValid = false;
    } else if (!/^\d+(\.\d{1,2})?$/.test(formData.currentInvestment)) {
      setInvestmentError('Current investment must be a valid number');
      isValid = false;
    } else if (parseFloat(formData.currentInvestment) < 0) {
      setInvestmentError('Current investment cannot be negative');
      isValid = false;
    }
  ...
  const openEditModal = (member) => {
    ...
    setFormData({
      ...
      currentBalance: String(member.balance ?? 0),
      currentInvestment: String(member.currentInvestment ?? 0),
  ...
  const updateFunds = async (balance, investment) => {
    try {
      // Get current funds
      const fundsSnap = await database.ref('Funds').once('value');
      const currentFunds = fundsSnap.val() || { totalFunds: 0, availableFunds: 0, investedFunds: 0 };
      ...
      // Update funds
      const updatedFunds = {
        totalFunds: (parseFloat(currentFunds.totalFunds) || 0) + totalBalance + totalInvestment,
        availableFunds: (parseFloat(currentFunds.availableFunds) || 0) + totalBalance,
        investedFunds: (parseFloat(currentFunds.investedFunds) || 0) + totalInvestment
      };
      
      await database.ref('Funds').set(updatedFunds);
      console.log('Funds updated successfully:', updatedFunds);
    } catch (error) {
  ...
  const submitAddMember = async () => {
    ...
    try {
      ...
      const currentInvestment = parseFloat(formData.currentInvestment) || 0;
      ...
      const memberData = {
        ...
        loans: parseFloat(formData.loans) || 0.0,
      ...
      // Update funds with the new balance and investment
      await updateFunds(currentBalance, currentInvestment);
      ...
      setSuccessMessage('Member added successfully! Funds have been updated and credentials have been sent to the member.');
  ...
  const submitEditMember = async () => {
    ...
    try {
      ...
      const currentBalance = parseFloat(formData.currentBalance) || 0;
      const currentInvestment = parseFloat(formData.currentInvestment) || 0;
      const previousBalance = parseFloat(editingMember.balance) || 0;
      const previousInvestment = parseFloat(editingMember.currentInvestment) || 0;
      ...
      // Update funds if balance or investment changed
      if (currentBalance !== previousBalance || currentInvestment !== previousInvestment) {
        const balanceDiff = currentBalance - previousBalance;
        const investmentDiff = currentInvestment - previousInvestment;
        
        if (balanceDiff !== 0 || investmentDiff !== 0) {
          await updateFunds(balanceDiff, investmentDiff);
        }
      }

      setSuccessMessage('Member updated successfully! Funds have been updated accordingly.');
      setSuccessModalVisible(true);
      closeModals();
      await fetchMembers();
    } catch (error) {
  ...
  const handleSuccessOk = async () => {
    ...
    if (pendingAdd) {
      try {
        ...
        console.log('Member credentials email sent successfully');
      } catch (error) {
        console.error('Error sending member credentials email:', error);
        // Don't show error to user since member was already created successfully
                ...
                <>
                  <div style={styles.formSection}>
                    ...
                    <input
                      ...
                      placeholder="Enter custom member ID (e.g., 5002)"
                      value={formData.memberId}
                      onChange={(e) => {
                        const numericText = e.target.value.replace(/[^0-9]/g, '');
                        handleInputChange('memberId', numericText);
                      }}
                      type="text"
                    />
                    {memberIdError && <span style={styles.errorText}>{memberIdError}</span>}
                    <div style={{ fontSize: '12px', color: '#64748b', marginTop: '4px' }}>
                      Enter a custom numeric ID for migration purposes
                    </div>
                  </div>
            ...
            <div>
              {mode === 'add' ? (
                <>
                  ...
                  <div style={styles.formSection}>
                    <label style={styles.formLabel}>
                      Current Balance<span style={styles.requiredAsterisk}>*</span>
                    </label>
                    <input
                      style={styles.formInput}
                      ...
                      min="0"
                    />
                    {balanceError && <span style={styles.errorText}>{balanceError}</span>}
                    <div style={{ fontSize: '12px', color: '#64748b', marginTop: '4px' }}>
                      This amount will be added to available funds
                    </div>
                  </div>

                  <div style={styles.formSection}>
                    <label style={styles.formLabel}>
                      Current Investment<span style={styles.requiredAsterisk}>*</span>
                    </label>
                    <input
                      style={styles.formInput}
                      ...
                      min="0"
                    />
                    {investmentError && <span style={styles.errorText}>{investmentError}</span>}
                    <div style={{ fontSize: '12px', color: '#64748b', marginTop: '4px' }}>
                      This amount will be added to invested funds
                    </div>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div style={styles.formSection}>
                    ...
                    />
                    <div style={{ fontSize: '12px', color: '#64748b', marginTop: '4px' }}>
                      Changes will update available funds
                    </div>
                  </div>

                  <div style={styles.formSection}>
                    <label style={styles.formLabel}>Current Investment</label>
                    <input
                      style={styles.formInput}
                      type="number"
                      step="0.01"
                    ...
                    />
                    <div style={{ fontSize: '12px', color: '#64748b', marginTop: '4px' }}>
                      Changes will update invested funds
                    </div>
                  </div>
                ...
                <tbody>
                  {paginatedData.map(m => (
                    <tr key={m.id} style={styles.tableRow}>
                      ...
                      <td style={styles.tableCell}>{toPeso(m.currentInvestment)}</td>
            ...
            <div style={{...styles.modalCard, maxWidth: '400px'}} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={{padding: '24px', textAlign: 'center'}}>
                ...
                <p style={{margin: '0 0 24px 0', color: '#64748b'}}>
                  Are you sure you want to register this new member? This will create their account, update the funds, and send them login credentials.
                </p>
              </div>
              <div style={styles.modalActions}>
                <button
