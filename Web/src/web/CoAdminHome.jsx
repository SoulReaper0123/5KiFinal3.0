...
const CoAdminHome = () => {
  ...
  useEffect(() => {
    const loadSection = async () => {
      const savedSection = localStorage.getItem('coAdminActiveSection');
      if (savedSection) {
    ...
    // Subscribe to pending counts for co-admin accessible sections
    const refs = {
  ...
  const loadChatSessions = () => {
    try {
      const savedChats = localStorage.getItem('coAdminAIChatSessions');
      if (savedChats) {
  ...
  const saveChatSessions = (sessions) => {
    try {
      localStorage.setItem('coAdminAIChatSessions', JSON.stringify(sessions));
  ...
  const handleSectionChange = async (section) => {
    ...
    await localStorage.setItem('coAdminActiveSection', section);
  ...
  const renderSection = () => {
    switch (activeSection) {
      ...
      default: return <Deposits key={`deposits-${sectionReloadCounter}`} />;
    }
  };
  ...
  const handleLogout = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      localStorage.removeItem('coAdminActiveSection');
      logout();
      navigate('/');
    }, 1500);
  };
  ...
  const handleAIQuery = async (userMessage) => {
    ...
    try {
      console.log('Processing co-admin AI query:', userMessage);
        ...
        {!isCollapsed && (
          <div style={styles.scrollContainer}>
            <div style={styles.adminContainer}>
              ...
              <div style={styles.adminSubtitle}>Co-Admin Portal</div>
        ...
        <div style={styles.fullOverlay}>
          <div style={styles.overlayContent}>
            ...
            <p style={styles.overlayText}>
              For optimal functionality and security, please access the Co-Admin Portal from a desktop or tablet device.
            </p>
          </div>
        </div>
      )}
      ...
      {logoutModalVisible && (
        <div style={styles.centeredModal}>
          <div style={styles.modalCardSmall}>
            ...
            <p style={styles.modalText}>Are you sure you want to log out of the Co-Admin Portal?</p>
