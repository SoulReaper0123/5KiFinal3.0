...
import { View, Text, TextInput, FlatList, TouchableOpacity, Image, ActivityIndicator, Modal, ScrollView, StyleSheet} from 'react-native';
...
const ApprovedPayments = () => {
  ...
  useEffect(() => {
    const fetchApprovedPayments = async () => {
      try {
        const snapshot = await database.ref('ApprovedPayments').once('value');
        if (snapshot.exists()) {
          ...
          setApprovedPayments(approvedPaymentsData);
        } else {
          setApprovedPayments([]); // Set empty if no data exists
        }
      } catch (err) {
        console.error('Error fetching approved payments:', err);
        setError('An error occurred while fetching approved payments. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (approvedPayments.length === 0) {
    return (
      <View>
        <Text style={{ textAlign: 'center', marginTop: 20 }}>No approved payments available.</Text>
      </View>
    );
  }
  const tableHead = ['Member ID', 'Payment Amount', 'Mode of Payment', 'Date Applied', 'Transaction ID', 'Proof OF Payment', 'Date Approved'];

  // Filter deposits based on search query
  const filteredApprovedPayments = approvedPayments.filter(item =>
  ...
  // Pagination logic
 const paginatedData = filteredApprovedPayments.slice(currentPage * pageSize, (currentPage + 1) * pageSize);
 const totalPages = Math.ceil(filteredApprovedPayments.length / pageSize);
